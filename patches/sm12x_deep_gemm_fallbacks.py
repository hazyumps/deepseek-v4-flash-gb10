...
def _fp8_mqa_logits_topk_triton(
    ...
    """T2: fused-Triton variant of the top-k fallback. Identical chunked
    top-k merge as the torch version; per-chunk logits come from the fused
    tf32 Triton MQA-logits kernel instead of a bf16 cuBLAS head-loop. Masking
    is kept explicit (all-valid cu_seqlen to the kernel -> raw logits, then the
    same global-offset masked_fill_) so only the raw GEMM changes."""
    from vllm.v1.attention.ops.deepseek_v4_ops.sm12x_mqa import fp8_mqa_logits_triton
...
def fp8_fp4_mqa_topk_indices(
    ...
    except Exception as exc:  # live-serving safety net
        logger.warning(
